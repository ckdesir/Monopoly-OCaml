...
let position_of_square board s =
  let rec accumulate_positions (squares : square list) =
    ...
    | [] -> []
    | h :: t -> if h.name = s 
      then h.position :: accumulate_positions t 
      else accumulate_positions t in
  let positions = accumulate_positions board.squares in
  if List.length positions = 0 then raise (UnknownSquare s) else positions

let type_of_square board s = 
  try
    let square = find_square board s in 
    let valid_types =
    ["Go"; "Jail/Just Visiting"; "Chance"; "Community Chest"; 
    "Street"; "Income Tax"; "Luxury Tax"; "Railroad"; "Utility"; 
    "Free Parking"; "Go to Jail"] in
    let find valid_type = square.stype = valid_type in
    try
      List.find find valid_types
    with
    | Not_found -> raise (UnknownType square.stype)
  with
...
let cost_of_square board s =
  let matcher square excp =
    match square.cost with
    | Some cost -> cost
    | None -> raise excp in
  let valid_types = 
    ["Street"; "Income Tax"; "Luxury Tax"; "Railroad"; "Utility"] in
...
let mortgage_of_square board s =
  let matcher square excp =
    ...
    | None -> raise excp in
  square_operation_helper board s ["Street"; "Railroad"; "Utility"] matcher
...
let sort_rent_tiers rent_tiers = 
  let compare_rent_tiers x y = 
    Option.compare Int.compare x.tier y.tier in
...
let cost_of_tier_0_rent board s =
  let matcher square excp =
    ...
    | Some rent_tiers -> 
      ...
      if Option.is_none tier_0.tier || Option.is_none tier_0.cost 
      then raise excp else tier_0.cost
    | None -> raise excp in
  match square_operation_helper board s ["Street"; "Railroad"] matcher with
  | Some cost -> cost
...
let cost_of_tier_1_rent board s =
  let matcher square excp =
    ...
    | Some rent_tiers -> 
      ...
      if Option.is_none tier_1.tier || Option.is_none tier_1.cost 
      then raise excp else tier_1.cost
    | None -> raise excp in
  match square_operation_helper board s ["Street"; "Railroad"] matcher with
  | Some cost -> cost
  | None -> raise (InvalidSquare (type_of_square board s, s))
...
let cost_of_tier_2_rent board s =
  let matcher square excp =
    ...
    | Some rent_tiers -> 
      ...
      if Option.is_none tier_2.tier || Option.is_none tier_2.cost 
      then raise excp else tier_2.cost
    | None -> raise excp in
  match square_operation_helper board s ["Street"; "Railroad"] matcher with
  | Some cost -> cost
  | None -> raise (InvalidSquare (type_of_square board s, s))
...
let cost_of_tier_3_rent board s =
  let matcher square excp =
    ...
    | Some rent_tiers -> 
      ...
      if Option.is_none tier_3.tier || Option.is_none tier_3.cost 
      then raise excp else tier_3.cost
    | None -> raise excp in
  match square_operation_helper board s ["Street"; "Railroad"] matcher with
  | Some cost -> cost
  | None -> raise (InvalidSquare (type_of_square board s, s))
...
let cost_of_tier_4_rent board s =
  let matcher square excp =
    ...
    | Some rent_tiers -> 
      ...
      if Option.is_none tier_4.tier || Option.is_none tier_4.cost 
      then raise excp else tier_4.cost
    | None -> raise excp in
...
let cost_of_tier_5_rent board s =
  let matcher square excp =
    ...
    | Some rent_tiers -> 
      ...
      if Option.is_none tier_5.tier || Option.is_none tier_5.cost 
      then raise excp else tier_5.cost
    | None -> raise excp in
  match square_operation_helper board s ["Street"] matcher with
  | Some cost -> cost
  | None -> raise (InvalidSquare (type_of_square board s, s))
